...
$sizeWidth = $sizeWidths[$this->size] ?? $sizeWidths['md'];
...
$posStyle = $positionStyles[$this->position] ?? $positionStyles['right'];
...
<div
    x-data
    @if($this->closeOnEscape) x-on:keydown.escape.window="$wire.close()" @endif
>
    @if($this->open)
            ...
            <div style="{{ $panelStyle }}">
                ...
                @if($this->title)
                    <div style="display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.5rem; border-bottom: 1px solid #e5e7eb;">
                        <h2 style="font-size: 1.125rem; font-weight: 600; color: #111827; margin: 0;">{{ $this->title }}</h2>
                        <button
                            wire:click="close"
